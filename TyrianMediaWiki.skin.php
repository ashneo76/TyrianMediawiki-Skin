                            ...
                            <ul class="collapse list-unstyled" id="pageActions">
                                ...
                                if (count($menuItemsList) > 0) {
                                    foreach ($menuItemsList as $linkName => $linkInfoArray) {
                                        ...
                                        echo '<li>' . $this->toIconizedText($linkText, $linkIcon) . '</li>';
                                    }
    ...
    private function nav($nav) {
        ...
        foreach ($nav as $topItem) {
            ...
            if (array_key_exists('sublinks', $topItem)) {
                ...
                if( $pageTitle ) {
                    $output .= '<li' . ($this->data['title'] == $topItem['title'] ? ' class="active"' : '')
                        . '><a href="' . ( $topItem['external'] ? $topItem['link'] : $pageTitle->getLocalURL() )
                        . '">' . $topItem['title'] . '</a></li>';
                }
            }
        }
